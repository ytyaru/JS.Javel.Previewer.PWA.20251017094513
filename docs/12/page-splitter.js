...
//    get elapsedMs {return this._.elapsedMs}
    async split(viewer) {
        ...
        if ('all'===this._.name) {
            this._.name = 'one';
            this._.loading.show();
//            const splitter = this._.types.get('one');
            let pages = this.#splitter.make(viewer, this._.parser.body.manuscript);
            if (0 < pages.length) {book.append(...pages);}
            this._.loading.update(this.#splitter.pages.length, this._.parser.body.progress.rate);
            while (0 < (pages = this.#splitter.make()).length) {
            //while (null !== (pages = splitter.make())) {
                //if (0 < pages.length) {book.append(...pages);}
                book.append(...pages);
                this._.loading.update(this.#splitter.pages.length, this._.parser.body.progress.rate);
                console.log('ページ数:', this.#splitter.pages.length, pages.length);
                await wait(0);
            }
            console.log('ページ生成完了!!');
            /*
            for await (pages of splitter.make()) {
//                book.append(...pages);
                if (0 < pages.length) {book.append(...pages);}
                this._.loading.update(this.#splitter.pages.length, this._.parser.body.progress.rate);
                await wait(0);
