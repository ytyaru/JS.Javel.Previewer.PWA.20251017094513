...
class PageSplitter {
    constructor(parser, loading, footer) {
        ...
        this._.types = new Map([['all', new AllPageSplitter(parser)], ['one', new OnePageSplitter(parser)]]);
        this._.name = this.typeNames[0];
        this._.interval = null;
        this._.intervalSec= 5;
        this._.allCalculating = false;
        this._.allFinished = false;
    }
    get typeNames() {return [...this._.types.keys()]}
    ...
    get calculating() {return 'one'===this._.name ? this.#splitter.calculating : this._.allCalculating;}
...
//        this.#splitter.init();
        ...
        if (book.querySelector('.cover')) {book.querySelector('.cover').remove();}
        console.log(this._.name);
        console.log(this.typeNames);
        console.log(this.#splitter);
        book.prepend(this.#splitter.makeCover());
//        book.prepend(this._.types[this._.name].makeCover());
        //book.prepend(this._.splitter.makeCover());
        */
        if ('all'===this._.name) {
            this._.loading.show();
            this._.allCalculating = true;
            this._.allFinished = false;
            book.querySelector('.page.cover').classList.add('show');
            // 表紙以降の本文
            for await (let page of this.#splitter.generateAsync(viewer)) {
            //for await (let page of this._.splitter.generateAsync(viewer)) {
            //for await (let page of this._.splitter.generateAsync(this._.O.viewer)) {
            //for await (let page of this._.splitter.generateAsync(book)) {
                console.log('ページ数:',page.dataset.page)
                book.appendChild(page);
                //this._.loading.update(parseInt(page.dataset.page), this._.parser.body.progress.rate);
                this._.loading.update(this.#splitter.pages.length, this._.parser.body.progress.rate);
                //this._.loading.update(this._.types[this._.name].pages.length, this._.parser.body.progress.rate);
...
//                Dom.q('[name="loading-rate"]').textContent = `${this._.parser.body.progress.rate.toFixed(100===this._.parser.body.progress.rate ? 0 : 1)}%`;
            ...
            if (viewer.querySelector('.back-cover')) {viewer.querySelector('.back-cover').remove();}
            //book.appendChild(this._.splitter.makeBackCover());
            book.appendChild(this.#splitter.makeBackCover());
            //book.appendChild(this._.types[this._.name].makeBackCover());
...
//                const pages = this._.splitter.make();
                //const pages = this._.types[this._.name].make();
                const pages = this.#splitter.make();
                ...
                //this._.footer.allPage = this._.types[this._.name].pages.length;
                //console.warn('this._.footer.allPage:', this._.footer.allPage, this._.splitter.pages);
                //console.warn('this._.footer.allPage:', this._.footer.allPage, this._.types[this._.name].pages);
...
//        const nextPage = this.#nowPage.nextElementSibling;
        if ('one'===this.type) {
...
//            if (!this.#splitter.finished && this._.footer.nowPage===this._.footer.allPage && !this._.pagingDisabled && this.#isSplitMode) {
            //if (!this._.splitter.finished && this._.footer.nowPage===this._.footer.allPage && !this._.pagingDisabled && this.#isSplitMode) {
            //if (!this.finished && this._.footer.nowPage===this._.footer.allPage && !this._.pagingDisabled && this.#isSplitMode) {
            //if (!this.finished && this._.footer.nowPage===this._.footer.allPage && !pagingDisabled && this.#isSplitMode) {
    ...
    //            console.log('未読込につき遷移無視する。');return
                ...
                //const bookInPages = document.querySelector('[name="book-in-pages"]');
